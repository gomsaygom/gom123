...
const port = 3000; // ⬅️ (기존 주석) (님이 만든 코드) 사용할 포트 번호 설정 (3000번으로 수정됨)
...
const saveRecentView = async (userId, accommodationId) => {
    try {
        ...
        console.log(`👀 최근 본 숙소 저장됨 (User: ${userId}, Acc: ${accommodationId})`);
    } catch (err) {
...
app.get('/accommodations/:id', async (req, res) => {
    ...
    //  1.  주소(URL)에서 ':id' 값을 꺼냅니다.
    const { id } = req.params; 

    // 로그인한 사람인지 확인해서 기록 남기기 (비회원은 기록 안 함)
    const authHeader = req.headers.authorization;
    if (authHeader) {
        try {
            const token = authHeader.split(' ')[1];
            const decoded = jwt.verify(token, JWT_SECRET_KEY);
            // 로그인한 유저라면 저장 함수 실행! (await 없이 실행만 시켜둠)
            saveRecentView(decoded.userId, id); 
        } catch (e) {
            // 토큰 만료 등 에러 나면 그냥 무시 (상세 페이지는 보여줘야 함)
        }
    }
...
const upload = multer(); // 기본 설정

// 8-1. 후기 작성하기 (POST /reviews)
// (로그인한 사용자만 가능 -> authMiddleware 사용)
// [수정] upload.none() 추가: 프론트에서 FormData로 보내도 req.body를 읽을 수 있게 해줍니다.
app.post('/reviews', authMiddleware, upload.none(), async (req, res) => {
    
    // 1. 작성자(나)의 ID를 토큰에서 꺼냅니다.
    // (authMiddleware가 토큰을 검증해서 req.user에 넣어줬다고 가정)
    const userId = req.user.userId || req.user.id; // [수정] 토큰 구조에 따라 유동적으로 가져오도록 안전장치 추가
    
// 2. 프론트에서 보낸 내용 받기 (const -> let으로 변경: 수정 가능하게)
    let { accommodation_id, rating, content } = req.body;

    console.log("📝 받은 데이터:", { accommodation_id, rating, content });

    // [임시 수정] 별점(rating)이 없으면(undefined) 강제로 5점으로 설정!
    if (rating === undefined || rating === "") {
        rating = 5; 
    }

    // 필수값 체크 (이제 rating은 5점이 기본이라 검사에서 뺐습니다)
    if (!accommodation_id || !content) {
        return res.status(400).json({ message: '숙소ID와 내용은 필수입니다.' });
    }
...
/* =========================================================
   ...
   ========================================================= */
// 수정할 때도 FormData를 쓸 수 있으니 upload.none()을 넣어줍니다.
app.put('/reviews/:id', authMiddleware, upload.none(), async (req, res) => {
    
    const { userId } = req.user; // 내 ID
    const { id: reviewId } = req.params; // 수정할 리뷰 ID
    
    // 수정할 내용 받기 (let으로 선언)
    let { rating, content } = req.body;

    // (유효성 검사) 내용이 아예 없으면 안 됨
    if (!content) {
        return res.status(400).json({ message: '수정할 내용을 입력해주세요.' });
    }

    try {
        // 1. 내 리뷰가 맞는지 확인 (그리고 숙소 ID도 알아내야 함 -> 평점 재계산용)
        const checkQuery = 'SELECT accommodation_id, rating FROM Review WHERE review_id = ? AND user_id = ?';
        const [reviews] = await dbPool.query(checkQuery, [reviewId, userId]);

        if (reviews.length === 0) {
            return res.status(403).json({ message: '리뷰를 찾을 수 없거나 수정 권한이 없습니다.' });
        }
        
        const accommodationId = reviews[0].accommodation_id;
        
        // (팁) 만약 프론트에서 별점을 안 보냈으면? -> 기존 별점 그대로 유지
        if (rating === undefined || rating === "") {
        ...
        const updateQuery = `
            ...
            WHERE review_id = ?
        `;
        await dbPool.query(updateQuery, [rating, content, reviewId]);

        // 3. (중요!) 별점이 바뀌었을 수도 있으니, 숙소 평균 평점 다시 계산!
        const updateScoreQuery = `
            UPDATE Accommodation a
            SET 
                rating = (SELECT AVG(rating) FROM Review WHERE accommodation_id = a.accommodation_id)
...
/* =========================================================
   🚀 8-3 API: 후기 삭제하기 (DELETE /reviews/:id)
   ========================================================= */
...
app.get('/users/recent', authMiddleware, async (req, res) => {
    ...
    try {
        // recently_viewed 테이블과 Accommodation 테이블을 합쳐서 정보 가져오기
        const sql = `
            SELECT 
                a.accommodation_id, 
                a.name, 
...
app.listen(port, async () => {
    try {
        ...
        console.log(`🎉 서버가 http://localhost:${port} 에서 실행 중입니다.`); // ⬅️ 3000번 포트로 실행
